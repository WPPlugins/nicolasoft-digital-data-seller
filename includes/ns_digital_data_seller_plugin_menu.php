...
{
	add_menu_page( __('Data sales','ns_digital_data_seller_plugin'), __('Data sales','ns_digital_data_seller_plugin'), 'manage_options', 'nsddsp_menu_download', 'nsddsp_draw_dummy_page' , '', 7);	
...
//保存処理 
function add_enable_nsddsp_mime_type($mimes) 
{
    //本来あるべきMIME Typeなど考えずに適当に書いてもあまり問題ない    
    $mimes['dat'] = 'application/octet-stream';
    return $mimes;
}
add_filter('upload_mimes', 'add_enable_nsddsp_mime_type');
...
add_action('init', nsddsp_create_required_files, 0);

//「ダウンロードファイル」のカスタム投稿一覧におけるカラムの設定
function post_id_downfile_columns($columns) 
{
    $columns['column_downfile_down_cnt'] = __('Number of downloads','ns_digital_data_seller_plugin');
    return $columns;
}

//「ダウンロードファイル」のカラムの表示内容を定義
//追加したカラムに「metaname」というカスタムフィールドの内容を表示させる
function add_post_id_downfile_column($column_name, $post_id) 
{
    if( $column_name == 'column_downfile_down_cnt' ) 
    {
...
add_filter( 'manage_edit-'.NSDDSP_POST_ID_DOWNFILE.'_columns', 'post_id_downfile_columns' );
add_action( 'manage_posts_custom_column', 'add_post_id_downfile_column', 10, 2 );

//「IPNデータ」のカラムの表示内容を定義
function post_id_ipndata_columns($columns) 
{
    $columns['column_ipndata_status'] = __('Email transmission status','ns_digital_data_seller_plugin');
    $columns['column_ipndata_payment_status'] = __('Payment status','ns_digital_data_seller_plugin');
    $columns['column_ipndata_downcnt_status'] = __('Number of downloads','ns_digital_data_seller_plugin');
    return $columns;
}

//「IPNデータ」のカラムの表示内容を定義
//追加したカラムに「metaname」というカスタムフィールドの内容を表示させる
function add_post_id_ipndata_column($column_name, $post_id) 
{
    if( $column_name == 'column_ipndata_status' ) 
    {
        $post_meta = get_post_meta($post_id, NSDDSP_METABOX_ID_IPN_DATA, true);
        if ( isset($post_meta) && $post_meta ) 
        {
    ...
    if( $column_name == 'column_ipndata_payment_status' ) 
    {
        $post_meta = get_post_meta($post_id, NSDDSP_METABOX_ID_IPN_DATA, true);
        if ( isset($post_meta) && $post_meta ) 
        {
    ...
    if( $column_name == 'column_ipndata_downcnt_status' ) 
    {
        $post_meta = get_post_meta($post_id, NSDDSP_METABOX_ID_IPN_DATA, true);
        if ( isset($post_meta) && $post_meta ) 
        {
...
add_filter( 'manage_edit-'.NSDDSP_POST_ID_IPNDATA.'_columns', 'post_id_ipndata_columns' );
add_action( 'manage_posts_custom_column', 'add_post_id_ipndata_column', 10, 2 );
