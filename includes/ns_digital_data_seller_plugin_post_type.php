...
function nsddsp_getDownFileUrl($post_id,$expire,$ipn_post_id)
{
    ...
    $token = hash('sha256', $down_url_token.'/'.strval($post_id).'/'.strval($expire));
    //$down_url = site_url('','http').'/?'.'nsddsp=1&download_id='.strval($post_id).'&expire='.strval($expire).'&ncl_token='.$token;
    $down_url = home_url('','http').'/?'.'nsddsp=1&download_id='.strval($post_id).'&expire='.strval($expire).'&ipn='.strval($ipn_post_id).'&ncl_token='.$token;
    return $down_url;
}

//PaypalからのItem Nameからダウンロードファイルのリストを求める
...
function enableDownFileUrl($post_id,$expire,$token) 
{
	$down_url_token = nsddsp_getTokenSrcWord();
    ...
    $check_token = hash('sha256', $down_url_token.'/'.strval($post_id).'/'.strval($expire));    
