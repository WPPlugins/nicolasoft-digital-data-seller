...
function nsddsp_process_paypalipn() 
{
    ...
    if ( ! is_wp_error( $response ) && strstr( $response['body'], 'VERIFIED' ) ) 
	{
		...
		{
			nsddsp_debug_log('$payment_status is not Completed $payment_status = '.$payment_status,false);	
		}
    }
    else 
    {
...
function nsddsp_process_download() 
{
	...
	if(enableDownFileUrl( $args['download_id'],$args['expire'],$args['token'] ) == false )
	{		
		nsddsp_debug_log('Error DownloadFile download_id='.$args['download_id'].' expire='.$args['expire'].' ip='.$ip_address.' ipn_post_id ='.$ipn_post_id, false);
